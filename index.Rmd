...
I tried to make classification visualizations. But I got an error every time. My R doesn't recognize the 'add_audio_analysis'. 
The error I get: Error in purrr::map(track_uri, get_tidy_audio_analysis, ...) : object 'track_uri' not found

And when I try without 'add_audio_analysis', I get the error: Error: `.x` must be a vector, not a function. 

I really don't know how to fix this. I have installed all the packages that are needed and I updated everything... I hope that maybe one of the peer reviewers have a solution for this or knows what I'm doing wrong. 
...
### Highest In The Room and Apocalypse are most different in tempo.
...
So I can conclude here that the two songs differs the most in tempo. It is almost the exact opposite. Still we both really like these songs. Maybe our tastes in genres are not that different, only the levels in the genres. It is a pity that I don't have these wrapped up lists from the other years that we've been together. Because it would be interesting to compare all those lists.
