...
spotify_2019_class %>%
    fit(spotify_2019_multinom, playlist ~ ., data = .) %>%
    pluck('fit') %>%
    coef(s = 0)
...
spotify_2019_class %>% 
    fit(spotify_2019_tree, playlist ~ ., data = .) %>% 
    pluck('fit') %>%
    summary
...
predict_forest <- function(split)
    fit(spotify_2019_forest, playlist ~ ., data = analysis(split)) %>% 
    predict(assessment(split), type = 'class') %>%
    bind_cols(assessment(split))
...
spotify_2019_class %>% 
    fit(spotify_2019_forest, playlist ~ ., data = .) %>% 
    pluck('fit') %>% 
