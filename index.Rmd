...
compmus_long_distance(apocalypse %>% 
    mutate(pitches = map(pitches, compmus_normalise, 'chebyshev')),
    highest %>% mutate(pitches = map(pitches, compmus_normalise, 'chebyshev')),
    feature = pitches, method = 'euclidean') %>% 
    ggplot(aes(x = xstart + xduration / 2, width = xduration, 
    y = ystart + yduration / 2, height = yduration, fill = d)) + 
    geom_tile() +
    scale_fill_continuous(type = 'viridis', guide = 'none') +
    labs(x = 'Apocalypse', y = 'Highest In The Room') +
    theme_minimal()
```

***
There are only two songs that appear in both our playlists. Those songs are Apocalypse by Cigarettes After Sex and Highest In The Room by Travis Scott. These two songs are not even close to each other. That's why I have put them together in a chromagram. 

### The timbre in Apocalypse by Cigarettes After Sex
